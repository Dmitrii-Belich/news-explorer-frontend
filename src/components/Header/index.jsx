...
export default function Header({popupOpen, isLoggedIn, onLogOut, isPopupOpen}) {
  ...
  return (
    <header
      className={`header ${(!isMainPage || scrolled) && !isPopupOpen && 'header_type_white'} ${isOpen && 'header_type_opened'}`}>
      <Container>
        <p className="header__name">NewsExplorer</p>
        {!isPopupOpen &&
        <button className={`header__close ${(!isMainPage || scrolled) && 'header__close_type_white'}`} onClick={() => {
          setIsOpen(!isOpen)
        }}/>}
        <nav className="header__nav">
          <Link className={`header__link ${isMainPage && 'header__link_type_active'}`} to="/">Главная</Link>
          {isLoggedIn && (
            <Link className={`header__link ${!isMainPage && 'header__link_type_active'}`} to="/saved-news">Сохранённые
              статьи</Link>)}
          {isLoggedIn
            ? (<button className="header__user" onClick={onLogOut}>{name}<SvgExit className="header__user-svg"/>
            </button>)
